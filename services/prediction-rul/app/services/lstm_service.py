...
class LSTMService:
    ...
    def save_model(self, save_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Modèle LSTM sauvegardé dans {save_path}")
            return True
            
        except Exception as e:
            logger.error(f"Erreur lors de la sauvegarde du modèle LSTM: {e}", exc_info=True)
            return False
    ...
    def load_model(self, load_path: str) -> bool:
        ...
        try:
            ...
            # Vérifier que c'est un modèle LSTM
            if checkpoint.get('model_type') != 'lstm':
                logger.error(f"Type de modèle incorrect: {checkpoint.get('model_type')}")
                return False
            
            # Récupérer les paramètres
            input_size = checkpoint.get('input_size')
            hidden_size = checkpoint.get('hidden_size', settings.lstm_hidden_size)
            num_layers = checkpoint.get('num_layers', settings.lstm_num_layers)
            dropout = checkpoint.get('dropout', settings.lstm_dropout)
            
            if input_size is None:
                logger.error("input_size manquant dans le checkpoint")
                return False
            ...
            self.is_trained = True
            logger.info(f"Modèle LSTM chargé depuis {load_path}")
            return True
            
        except Exception as e:
            logger.error(f"Erreur lors du chargement du modèle LSTM: {e}", exc_info=True)
            return False
